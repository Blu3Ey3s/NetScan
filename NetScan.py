...
DEFAULT_MAX_CONCURRENT_CONNECTIONS = 300
...
async def scan_port(ip, port, semaphore, output_file):
    ...
    result = None  # Инициализируем результат перед использованием
...
def main():
    ...
    parser = argparse.ArgumentParser(description="High-speed port scanner like Masscan written in Python PL")
    ...
    parser.add_argument("-o", "--output", type=str, help="Output file to save scan results (output file should be a simple txt file like result.txt)")
    ...
    try:
        ...
        print(f"{Fore.BLUE}IP address/'es {args.host}\nPorts/Port: {args.ports}\nConcurrent connections: {args.threads}{Style.RESET_ALL}")
    ...
    print('-' * 50)
    print(f"\n{Fore.GREEN}Scan completed in {int(minutes)} minutes and {int(seconds)} seconds{Style.RESET_ALL}")
